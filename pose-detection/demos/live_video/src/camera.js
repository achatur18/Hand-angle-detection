...
export class Camera {
  ...
  angle(k1, k2, k3, h){
    let m1=99999999999999
    if ((k1.x-k2.x)!=0){
      m1 = (k1.y-k2.y)/(k1.x-k2.x)
    }

    let m2=99999999999999
    if ((k3.x-k2.x)!=0){
      m2 = (k3.y-k2.y)/(k3.x-k2.x)
    }

    let deg = Math.atan((m1-m2)/(1+m1*m2))*57.2958
  ...
  drawKeypoints(keypoints) {
    ...
    let angr = this.angle(keypoints[6], keypoints[8], keypoints[10], 'right')
    let angl = this.angle(keypoints[5], keypoints[7], keypoints[9], 'left')

    // console.log("left", angl)
    // console.log("right", angr)

    document.getElementById('left').innerHTML = "Left: "+angl;
    document.getElementById('right').innerHTML = "Right: "+angr;
